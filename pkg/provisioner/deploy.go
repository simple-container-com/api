...
func (p *provisioner) initProvisioner(ctx context.Context, params api.StackParams) (*api.ConfigFile, *api.Stack, api.Provisioner, error) {
	...
	p.log.Debug(ctx, "🔍 Reconciliation check: Environment=%s, Parent=%t", params.Environment, params.Parent)

	if params.Environment != "" && !params.Parent {
		p.log.Debug(ctx, "🔧 Running reconciliation for client stack deployment...")
		if stacks, err := p.stacks.ReconcileForDeploy(params); err != nil {
			...
			p.stacks = *stacks
			p.log.Debug(ctx, "✅ Reconciliation completed successfully")
		}
	} else {
		p.log.Debug(ctx, "⚠️  Skipping reconciliation - this is a parent stack or no environment specified")
		if params.Parent {
			p.log.Debug(ctx, "🏗️  Parent stack - secrets should have been revealed during parent repository setup")
		}
	}
