...
func AdoptPrivateBucket(ctx *sdk.Context, stack api.Stack, input api.ResourceInput, params pApi.ProvisionParams) (*api.ResourceOutput, error) {
	...
	if bucketCfg.BucketName == "" {
		return nil, errors.Errorf("bucketName is required when adopt=true for resource %q", input.Descriptor.Name)
	}
	...
	params.Log.Info(ctx.Context(), "adopting existing GCS bucket %q and creating S3 interoperability", bucketCfg.BucketName)

	// Import existing bucket into Pulumi state
	bucket, err := storage.NewBucket(ctx, bucketName, &storage.BucketArgs{
		Name:     sdk.String(bucketCfg.BucketName),
	...
	}, append(opts, sdk.Import(sdk.ID(bucketCfg.BucketName)))...)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to import GCS bucket %q", bucketCfg.BucketName)
	}
	...
	params.Log.Info(ctx.Context(), "creating service account for adopted bucket %q", bucketCfg.BucketName)
	...
	sa, err := serviceaccount.NewAccount(ctx, serviceAccountName, &serviceaccount.AccountArgs{
		...
		DisplayName: sdk.String(fmt.Sprintf("Service account for bucket %s", bucketCfg.BucketName)),
	}, opts...)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to create service account for bucket %q", bucketCfg.BucketName)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to grant bucket access to service account for %q", bucketCfg.BucketName)
	}

	params.Log.Info(ctx.Context(), "creating HMAC key for adopted bucket %q", bucketCfg.BucketName)
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to create HMAC key for bucket %q", bucketCfg.BucketName)
	}
	...
	params.Log.Info(ctx.Context(), "successfully adopted GCS bucket %q with S3 interoperability", bucketCfg.BucketName)
